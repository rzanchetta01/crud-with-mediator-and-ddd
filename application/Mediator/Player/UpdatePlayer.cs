...
public record UpdatePlayerCommand(PlayerDto PlayerDto) : IRequest<ResponseTemplate>;

public class UpdatePlayerHandler : IRequestHandler<UpdatePlayerCommand, ResponseTemplate>
{
    ...
    public async Task<ResponseTemplate> Handle(UpdatePlayerCommand request, CancellationToken cancellationToken)
    {
    ...
    private static Tuple<bool, List<string>> Validate(domain.Entities.Player entity)
    {
        ...
        var isPassed = errors.Count > 0;
        return Tuple.Create(isPassed, errors);
    }
}